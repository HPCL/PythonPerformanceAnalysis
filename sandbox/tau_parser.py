    ...
    def parse(cls, dir_path, filenames=None, trial=None):
        ...
        for filename in sorted(filenames,
            ...
            with open(file_path) as fin:
                ...
                interval = pd.read_table(mm, nrows=interval_count, delim_whitespace=True,
                ...
                atomic = pd.read_table(mm, names=['Count', 'Maximum', 'Minimum', 'Mean', 'SumSq'],
        ...
        interval_df = pd.concat(intervals, keys=indices)
        interval_df.index.rename(['Node', 'Context', 'Thread', 'Timer'], inplace=True)
        atomic_df = pd.concat(atomics, keys=indices)
